...
namespace Azimuth
{
	public class Config
	{
		...
		public static VALUE? Get<VALUE>(string _category, string _key)
		{
			...
			{
				// Attempt to get the vector3s value from the config, then try to change it to the correct type
			...
			{
				// Attempt to get the INTS value from the config, then try to change it to the correct type
		...
		private void Load()
		{
			...
			using(StreamReader reader = new StreamReader(FilePath))
			{
				...
				// line will only be null if we reach the end of the file
				// empty lines are not considered null
